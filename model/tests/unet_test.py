...
class UnetTest(tf.test.TestCase):
    ...
    def test_output_shape(self):
        ...
        image = tf.ones(shape)
        self.unet.build()
        self.asertEqual(self.unet.Model.predict(image).shape, shape)
    ...
    def test_load_data(self, mock_data_loader):
        ...
        self.assertEqual(self.unet.train_dataset.element_spec[0].shape, shape)
        self.assertEqual(self.unet.test_dataset.element_spec[0].shape, shape)
